/*
    1. 콜백함수로 화살표 함수를 사용하게 되면
        해당 화살표 함수가 사용하는 곳에 따라 this의 의미가 정해진다.

    화살표 함수: 렉시컬 this(화살표 함수는 자신만의 this를 가지지 않음)를 상위 스코프에서 상속받아
                화살표 함수가 정의된 시점의 상위 스코프(showMovieList)에서의 this를 의미하게 된다.
    단순 익명 함수: 자체 this의 의미가 바인딩되어 있다.
                    콜백 함수로 쓰일 때는 기본적으로 global(전역) 객체를 의미하게 된다.
*/
...
// 객체를 만들 용도가 아닌, 오직 함수로만 사용할 함수를 정의하는데 사용 가능
const arrowFunc = () => {};
...
// this와 같이 상위 환경의 arguments가 아닌 상위 환경의 arguments(전역 객체의 arguments)가 나옴
